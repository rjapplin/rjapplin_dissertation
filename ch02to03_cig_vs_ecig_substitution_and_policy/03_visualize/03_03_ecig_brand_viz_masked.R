brand_info <- read_csv("ecig_brands.csv")
...
ggsave(paste0(direcs$pap.fig, "nielsen/brand_shares/concentration_7467.png"))
...
ggsave(paste0(direcs$pap.fig, "nielsen/brand_shares/brand_shares_sales_7467.png"), g)
ecigs %>%
  ...
  scale_color_manual(values = unlist(color_pal)[c(1:9)] %>% unname(),
                     name = "Brand") +
  theme_economist_white(gray_bg = FALSE) + xlab("Year") +
  ylab("Market Share (By Volume)") +
  theme(
    legend.position = "bottom"
  ) -> g
ggsave(paste0(direcs$pap.fig, "nielsen/brand_shares/brand_shares_vol_7467.png"), g)
