...
sm$coefficients %>% 
  ...
  filter(Feature %in% c("treated_after", "r", "I(treated_after * r)", 
                        "number_of_reviews", 
                        "I(30 - available_030)",
  ...
  mutate(Feature = case_when(
    ...
    Feature == "I(treated_after * r)" ~ "rating",
    .default = Feature
  )) %>%
  dplyr::rename(OLS = value) %>%
  dbWriteTable(iaan, "MODEL.PRICE_OLS_BY_RATE_FORMATTED", ., overwrite = T)
...
sm$coefficients %>%
  ...
  dbWriteTable(iaan, "MODEL.PRICE_OLS_BY_RATE_COEFS", ., overwrite = T)
...
sm$coefficients %>% 
  ...
  dplyr::rename(OLS = value) %>%
  dbWriteTable(iaan, "MODEL.REAL_PRICE_OLS_BY_RATE_FORMATTED", ., overwrite = T)
...
sm$coefficients %>%
  ...
  rownames_to_column(var = "Feature") %>%
  select(Feature, Estimate) %>%
  dbWriteTable(iaan, "MODEL.REAL_PRICE_OLS_BY_RATE_COEFS", ., overwrite = T)
...
for(i in 1:100){
  ...
  dbExecute(iaan,
            "CREATE TABLE IF NOT EXISTS [MODEL.PRICE_IV_BY_RATE_LOOP_RES] (
            Feature TEXT,
            Estimate REAL,
            SE REAL,
            t REAL,
  ...
  m <- felm(
    ...
    data = dbReadTable(iaan, "DATA.ANALYSIS_SAMPLE") %>%
      select(price, price1, price2, price3, r, bathrooms, bedrooms, capacity,
  ...
  sm$coefficients %>% 
    as.data.frame() %>%
    rownames_to_column(var = "Feature") %>%
    dplyr::filter(Feature %in% c("number_of_reviews", "r", 
                                 "price1", "price2", "price3", "bathrooms", "bedrooms",
                                 "capacity", "covid", "AC7", "AC51", "AC72") |
                    grepl("fit", Feature)) %>%
    dplyr::mutate(n = case_when(
      ...
      Feature == "`r`" ~ 2,
      Feature == "`I(treated_after * r)(fit)" ~ 3,
      Feature == "`I(30 - available_030)(fit)`" ~ 4,
      .default = 999
    )) %>%
    dplyr::arrange(n) %>%
    dplyr::select(-n) %>%
    ...
    plyr::rbind.fill(
      ...
      data.frame(Feature = "rating Cond. F Stat",
                 Estimate = round(condf[3], 3)),
      data.frame(Feature = "treated*rated Cond. F Stat",
                 Estimate = round(condf[4], 4)),
    ...
    dplyr::mutate(Feature = case_when(
      ...
      Feature == "`r(fit)`" ~ "rated",
      Feature == "`I(treated_after * r)(fit)`" ~ "treated*rated",
      .default = Feature
    )) %>%
    dplyr::mutate(iteration = i,
                  seed = seed) %>%
    dbWriteTable(iaan, "MODEL.PRICE_IV_BY_RATE_LOOP_RES", ., append = T)
  print(i)
  rm(m, sm, condf)
  gc()
...
for(i in 1:100){
  ...
  dbExecute(iaan,
            "CREATE TABLE IF NOT EXISTS [MODEL.REAL_PRICE_IV_BY_RATE_LOOP_RES] (
            Feature TEXT,
            Estimate REAL,
            SE REAL,
            t REAL,
    ...
    + bathrooms + bedrooms + capacity + covid +
      ...
      (treated_after | I(30 - available_030) | r | I(treated_after*r) ~  + as.factor(host_response_time)
       + host_response_rate + 
         ...
         I(descrip_word_count*r) + I(loc_overview_word_count*r)),
    data = dbReadTable(iaan, "DATA.ANALYSIS_SAMPLE") %>%
      select(real_price, real_price1, real_price2, real_price3, r, 
  ...
  sm$coefficients %>% 
    ...
    dplyr::mutate(iteration = i,
                  seed = seed) %>%
    dbWriteTable(iaan, "MODEL.REAL_PRICE_IV_BY_RATE_LOOP_RES", ., append = T)
  print(i)
  rm(m, sm, condf)
  gc()
...
#FE Target - Price-----------------------------------------------------------------------
m <- felm(
  price ~ treated_after + r + I(treated_after*r) + I(30 - available_030) +
    price1 + price2 + price3 + bathrooms + bedrooms + capacity + covid +
    number_of_reviews +
    as.factor(listing_type) + as.factor(room_type) + 
...
sm$coefficients %>% 
  ...
  dplyr::rename(OLS = value) %>%
  dbWriteTable(iaan, "MODEL.PRICE_FE_BY_RATE_FORMATTED", ., overwrite = T)
...
sm$coefficients %>%
  ...
  rownames_to_column(var = "Feature") %>%
  select(Feature, Estimate) %>%
  dbWriteTable(iaan, "MODEL.PRICE_FE_BY_RATE_COEFS", ., overwrite = T)
...
m <- felm(
  real_price ~ treated_after + r + I(treated_after*r) + I(30 - available_030) +
...
sm$coefficients %>% 
  ...
  dbWriteTable(iaan, "MODEL.REAL_PRICE_FE_BY_RATE_FORMATTED", ., overwrite = T)
...
sm$coefficients %>%
  ...
  rownames_to_column(var = "Feature") %>%
  select(Feature, Estimate) %>%
  dbWriteTable(iaan, "MODEL.REAL_PRICE_FE_BY_RATE_COEFS", ., overwrite = T)
...
for(i in 1:100){
  ...
  dbExecute(iaan,
            "CREATE TABLE IF NOT EXISTS [MODEL.PRICE_FEIV_BY_RATE_LOOP_RES] (
            Feature TEXT,
            Estimate REAL,
            SE REAL,
            t REAL,
  ...
  sm$coefficients %>% 
    ...
    dplyr::mutate(Feature = case_when(
      ...
      Feature == "`I(treated_after * r)(fit)`" ~ "treated*rating",
      Feature == "`I(30 - available_030)(fit)`" ~ "booked_nights",
      .default = Feature
    )) %>%
    dplyr::mutate(iteration = i,
                  seed = seed) %>%
    dbWriteTable(iaan, "MODEL.PRICE_FEIV_BY_RATE_LOOP_RES", ., append = T)
  print(i)
  rm(m, sm, condf)
  gc()
...
for(i in 1:100){
  ...
  dbExecute(iaan,
            "CREATE TABLE IF NOT EXISTS [MODEL.REAL_PRICE_FEIV_BY_RATE_LOOP_RES] (
            Feature TEXT,
            Estimate REAL,
            SE REAL,
            t REAL,
  ...
  m <- felm(
    real_price ~ real_price1 + real_price2 + real_price3 + bathrooms + bedrooms + 
      capacity + covid +
      number_of_reviews + host_is_superhost +
      as.factor(listing_type) + as.factor(room_type) + 
      AC7 + AC51 + AC72 + as.factor(year) + as.factor(month) | KEY_LIST_ID |
  ...
  sm$coefficients %>% 
    ...
    dplyr::mutate(iteration = i,
                  seed = seed) %>%
    dbWriteTable(iaan, "MODEL.REAL_PRICE_FEIV_BY_RATE_LOOP_RES", ., append = T)
  print(i)
  rm(m, sm, condf)
  gc()
...
rm(all_listings, i, samp_listings, seed)
